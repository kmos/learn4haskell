...
-}
firstHalf :: [a] -> [a]
firstHalf l = take (getHalfLength l) l
  where
    getHalfLength :: [a] -> Int
    getHalfLength list = div (length list) 2
...
-}
takeEven :: [Int] -> [Int]
takeEven list = removeSnd $ filterEven $ combineWithConsecutiveNumbers list
  where
    removeSnd :: [(Int, Int)] -> [Int]
    removeSnd l = map(\(x,_) -> x) l

    filterEven :: [(Int,Int)] -> [(Int,Int)]
    filterEven pairList = filter(\(_,y) -> even y) pairList

    combineWithConsecutiveNumbers :: [Int] -> [(Int,Int)]
    combineWithConsecutiveNumbers l = zip l (genFiniteNumbers l)

    genFiniteNumbers :: [Int] -> [Int]
    genFiniteNumbers l = take (length l) [0..]
...
contains x l = filter(\sublist -> elem x sublist) l
...
rotate x list
  | list == [] = []
  | x < 0 = []
  | otherwise = drop x (take ((length list)+x) (cycle list))
...
-}
rewind :: [a] -> [a]
rewind list = inverse list
  where
    inverse :: [a] -> [a]
    inverse [] = []
    inverse l = [last l] ++ (inverse (init l))
